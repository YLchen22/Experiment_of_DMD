...
%%% add noise on data
mag = mean(abs(data), 'all'); mag = 1;

noise = randn(size(data))*1e-4;
% noise = eye(size(data)) * 1e-4;
data = data + noise;
...
eval_on = [];   eval_ons = [];   eval_sota = []; eval_dmd = [];
% online iteration
for i = 1: steps-init
    % data preparation
    Dt = data(:, 1:init+i);
    ...
    new_U = Dt * new_BU;
    [Q_on, R] = qr(new_U, 'econ');
    B = new_BU / R; % theoretically, Q_on = Dt * B

    err_rcond(i) = cond(R);
    err_newu(i) = norm(new_U - Q_on * R, 'fro');
    err_q(i) = norm(Q_on - Dt*B, 'fro');
    err_qpinv(i) = norm(Q_on - Dt*pinv(Dt)*Q_on, 'fro');

    [Qdata, ~] = qr(Dt, 'econ');
    err_dind(i) = norm(Dt - Qdata * Qdata' * Dt);
    err_qind(i) = norm(Q_on - Qdata * Qdata' * Q_on);
    ...
    eval_on = [eval_on, D_on];
    eval_ons = [eval_ons, D_ons];
    eval_dmd = [eval_dmd, D_dmd];
    eval_sota = [eval_sota, D_sota];
end
...
for i = 1:9
    subplot(3,3,i)
    hold on
    plot(abs(eval_on(i, :) - D(i)))
    plot(abs(eval_ons(i, :) - D(i)))
    plot(abs(eval_dmd(i, :) - D(i)))
    plot(abs(eval_sota(i, :) - D(i)))
    xlabel('Iteration')
    ylabel('Error')
    yscale log
...
plot(err_vec_sota)
yscale log
legend('Online algorithm', 'Computational online algorithm', 'DMD', 'direct', 'location', 'best')
...
plot(err_q)
plot(err_qpinv)
legend('rcond', 'error of eigvec', 'error of Q', 'error of Q through pinv')
yscale log
