...
for i = 1:iter
    ...
    err_pinv(n, i) = norm(pinv(lambda(n) * eye(dim) - A_approx) - inv_mat, 'fro');

    space_err(i) = norm(ortho_projection(U, right), 'fro');
    space_ref(i) = norm(ortho_projection(P, right), 'fro');
