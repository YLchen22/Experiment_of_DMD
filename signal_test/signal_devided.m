...
%%% double length of time of the system snapshots
x=0:0.01:5; m=length(x); %空间
t=0:0.1:15; n=length(t); %时间
Fs=1/(t(2)-t(1)); %采样频率
[X,T]=meshgrid(x,t);
...
figure()
imagesc(full_time_sys)
colorbar(); title('full time sys');
...
steps = 1 + (n-1)*3;  % prediction time steps (start at t = 0)
ampl = diag(pinv(tilde_Phi) * U_xt(:, 1));    % r*r
evol = tilde_Lambda .^ (0: steps-1);    % a trick to define vandermonde matrix
recov_dmd = real(tilde_Phi * ampl * evol);

matdisp({full_time_sys, recov_dmd})
...
%%% DMD system reconstruction
steps = 2 * n;  % prediction time steps (start at t = 0)
ampl = diag(pinv(tilde_Phi) * U_xt(:, 1));    % r*r
evol = tilde_Lambda .^ (0: steps-1);    % a trick to define vandermonde matrix
recov_dmd = real(tilde_Phi * ampl * evol);
