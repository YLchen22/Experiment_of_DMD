...
x=0:0.01:5; m=length(x); % space
t=0:0.05:5; N=length(t); % time
Fs=1/(t(2)-t(1));%采样频率
[X,T]=meshgrid(x,t);
...
U_tx = 1.2*exp(-0.5*T) .* sin(2*pi*(X+2*T)) ...
       ...
       + 1.1 + 0.1*rand(N,m);
% transpose observation to fit DMD arrangement
U_xt=U_tx';
...
[Q, R] = qr(Y); % the complexity here is the same as SVD
tilde_A = Z * pinv(R) * Q'; 
% tilde_A = AQQ' \approx= A Y R^-1 Q' = Z R^-1 Q', becuase Q=Y/R has error
% as a result, seems this method doesn't work
A = Z / Y;

[tilde_eigvec, tilde_eigval] = eig(tilde_A);
[eigvec, eigval] = eig(A);


function matcompare(A, B)
    minvalue = min([A, B], [], "all");
    maxvalue = max([A, B], [], "all");
    A = (A - minvalue) / (maxvalue - minvalue);
    B = (B - minvalue) / (maxvalue - minvalue);
    figure()
    subplot(121)
    imshow(A, 'Colormap', cool, 'InitialMagnification', 'fit'); title('A')
    subplot(122)
    imshow(B, 'Colormap', cool, 'InitialMagnification', 'fit'); title('B')
end
